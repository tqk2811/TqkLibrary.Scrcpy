...
$buildIndex="00"
$p=""#buildDay=-build$($buildDay);buildIndex=$($buildIndex)".Trim()
...
function NugetPack
{
    ...
    for ($i=0; $i -lt $numOfArgs; $i++)
    {
        ...
        $result = RunCommand "Remove-Item -Recurse -Force .\bin\ReleaseNoScreenShot\**" `
            "dotnet build .\$($args[$i]).csproj -c ReleaseNoScreenShot" `
            "nuget pack .\$($args[$i]).nuspec -OutputDirectory .\bin\ReleaseNoScreenShot -p 'id=$($args[$i]).ViewOnly;$($p)'"
...
function NugetPush
{
    ...
    for ($i=0; $i -lt $numOfArgs; $i++)
    {
        ...
        iex "dotnet nuget push .\bin\ReleaseNoScreenShot\*.nupkg --api-key $($key) --source https://api.nuget.org/v3/index.json"
    }
}
